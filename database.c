...
void delete_customer(HashTable *table, const char *email) {
    ...
    for (Node *current = table->buckets[index]; current != NULL; prev = current, current = current->next) {
...
void list_customers(HashTable *table) {
    for (Customer *current = table->head; current != NULL; current = current->next) {
...
void free_table(HashTable *table) {
    for (int i = 0; i < 256; i++) {
        for (Node *node = table->buckets[i]; node != NULL;) {
            Node *temp = node;
            node = node->next;
